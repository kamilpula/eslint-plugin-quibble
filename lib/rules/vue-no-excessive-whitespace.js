...
export default {
  ...
  create(context) {
    function handleNodeArgument(node, argument = null) {
      if (argument === null) {
        const value = getNodeValue(node)

        const classListNoWhitespace = removeWhitespace(value)

        if (value !== classListNoWhitespace) {
          context.report({
            node,
            loc: node.loc,
            messageId: 'excessive-whitespace-in-static-class',
            fix: fixer => fixer.replaceTextRange(node.value.range, `"${classListNoWhitespace}"`),
          })
        }
      }
      else {
        switch (argument.type) {
          case 'ObjectExpression':
            break
          case 'Literal':
            handleLiteralArgument(argument)
            break
          default:
            break
        }
    ...
    function handleLiteralArgument(argument) {
      const classList = argument.value
      if (!classList)
        return
      ...
      if (classList !== classListNoWhitespace) {
        context.report({
          ...
          loc: argument.loc,
          messageId: 'excessive-whitespace-in-static-class',
          fix: fixer => fixer.replaceTextRange(argument.range, `"${classListNoWhitespace}"`),
        })
      }
    }
    ...
    const scriptVisitor = {
      CallExpression(node) {
        ...
        node.arguments.forEach((argument) => {
          handleNodeArgument(node, argument)
        })
      },
    }
